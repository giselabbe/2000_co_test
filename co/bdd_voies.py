...
def flatten_capacite_classe_for(classe_id: str, voie_id: str, data: Dict) -> List[Dict]:
    """
    ...
    if voie_id not in cls_node:
        raise ValueError(f"Voie '{voie_id}' absente sous la classe '{classe_id}'.")

    out: List[Dict] = []
    for key, props in cls_node[voie_id].items():
        if "label" not in props or "rang" not in props:
            raise ValueError(f"Entrée '{key}' incomplète: 'label' et 'rang' requis.")
        rang = int(props["rang"])
        if not (1 <= rang <= 5):
            raise ValueError(f"Rang invalide {rang} pour '{key}' (doit être 1..5).")
...
def flatten_peuple_for(peuple_id: str, dict_capacites: Dict) -> List[Dict]:
    """
...
def upsert_all_capacite_classe_from_yaml(conn: sqlite3.Connection, data: Dict):
    """
